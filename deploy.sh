...
if [[ -n "${GS_MINECRAFT_RCON_PASSWORD:-}" ]]; then
    ...
    cat > "$ENV_FILE" <<EOF
# Minecraft
RCON_PASSWORD=${GS_MINECRAFT_RCON_PASSWORD}
OPS=${GS_MINECRAFT_OPS:-}
MOTD=${GS_MINECRAFT_MOTD:-TurtleServer Minecraft}
MAX_PLAYERS=${GS_MINECRAFT_MAX_PLAYERS:-10}
DIFFICULTY=${GS_MINECRAFT_DIFFICULTY:-normal}
MODE=${GS_MINECRAFT_MODE:-survival}
MEMORY=${GS_MINECRAFT_MEMORY:-4G}

# Don't Starve Together
DST_CLUSTER_TOKEN=${GS_DST_CLUSTER_TOKEN:-}
DST_CLUSTER_NAME=${GS_DST_CLUSTER_NAME:-TurtleServer DST}
DST_CLUSTER_PASSWORD=${GS_DST_CLUSTER_PASSWORD:-}
DST_CLUSTER_DESCRIPTION=${GS_DST_CLUSTER_DESCRIPTION:-Don't Starve Together on TurtleServer}
DST_MAX_PLAYERS=${GS_DST_MAX_PLAYERS:-6}
EOF
    chmod 600 "$ENV_FILE"
